...
def refine_corners_with_math(mask: np.ndarray, nn_corners: np.ndarray, radius: int = 40) -> np.ndarray:
    ...
    for nn_c in nn_corners:
        ...
        max_curvature = -1
        best_corner = nn_c
        # step : on compare à des voisins un peu éloignés sur le contour plutôt qu'aux pixels adjacents, pour un calcul d'angle plus stable
        step = max(5, N // 100) 
...
def calculate_iou(pred_mask, true_mask, threshold=0.5):
    ...
    if union == 0:
        # aucun pixel positif ni dans la prédiction ni dans la vérité terrain : on considère que c'est un accord parfait
...
def evaluate_model(weights_path="reseau_de_neurones/unet_puzzle_weights.pth", num_test_samples=500):
    ...
    # Chargement du modèle entraîné, verrouillé en mode évaluation (eval() désactive le comportement "entraînement" des couches BatchNorm/Dropout)
    model = PuzzleUNet().to(device)
    model.load_state_dict(torch.load(weights_path, map_location=device))
    model.eval()

    # Nouveau jeu de données, jamais vu pendant l'entraînement, pour mesurer la vraie capacité de généralisation du modèle
    ...
    # no_grad() : on ne fait que de l'inférence, pas d'entraînement, donc pas besoin de calculer les gradients
    with torch.no_grad():
        for inputs, targets in test_loader:
            inputs = inputs.to(device)
            targets = targets.to(device)  # targets contient 2 canaux : masque et heatmap

            preds = model(inputs)

            # canal 0 = masque de segmentation
...
def visualize_predictions(dataset, model, num_samples=3):
    ...
    with torch.no_grad():
        for i, idx in enumerate(indices):
            ...
            # masque prédit et coins détectés superposés en rouge
